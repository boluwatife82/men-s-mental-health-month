/* ============================================================
   seed.js — fills the database with all seeded content
   Run: node seed/seed.js
   Safe to run multiple times (checks before inserting)
============================================================ */
require('dotenv').config({ path: '../.env' });
process.env.DB_PATH = './data/weightwecarry.db';
...
/* ============================================================
   INSERT FUNCTIONS — skip if already seeded
============================================================ */
function seedStories() {
  const existing = db.prepare(`SELECT COUNT(*) as n FROM stories WHERE source = 'seeded'`).get().n;
  if (existing > 0) { console.log('  ⏭  Stories already seeded, skipping.'); return; }
  ...
  const insertMany = db.transaction((rows) => {
    for (const s of rows) insert.run(s.content, s.burden, s.country);
  });
  insertMany(STORIES);
...
function seedLetters() {
  const existing = db.prepare(`SELECT COUNT(*) as n FROM letters WHERE source = 'seeded'`).get().n;
  if (existing > 0) { console.log('  ⏭  Letters already seeded, skipping.'); return; }

  const insert = db.prepare(`
    INSERT INTO letters (title, body, from_line, burden, source, hearts, approved)
    VALUES (?, ?, ?, ?, 'seeded', ?, 1)
  `);
  const insertMany = db.transaction((rows) => {
    for (const l of rows) insert.run(l.title, JSON.stringify(l.body), l.from_line, l.burden, l.hearts);
  });
  insertMany(LETTERS);
...
function seedMessages() {
  const existing = db.prepare(`SELECT COUNT(*) as n FROM messages WHERE source = 'seeded'`).get().n;
  if (existing > 0) { console.log('  ⏭  Messages already seeded, skipping.'); return; }

  const insert = db.prepare(`
    INSERT INTO messages (content, burden, source, hearts)
    VALUES (?, ?, 'seeded', ?)
  `);
  const insertMany = db.transaction((rows) => {
    for (const m of rows) insert.run(m.content, m.burden, m.hearts);
  });
  insertMany(MESSAGES);
